      ...
      <v-layout>
        <v-dialog v-model="dialog.show">
          ...
          <v-card>
            ...
            <v-card-text>
              <div class="subheading">您真的要删除账号吗？该操作无法撤销。</div>
              <v-form v-model="dialog.valid" ref="dialog">
                <v-text-field
                  v-model.trim="dialog.username"
                  :rules="[v => v === this.user.username || '']"
                  prepend-icon="account_circle"
                  label="请输入用户名"
        ...
        <v-btn flat @click="$store.commit('edit', false); Object.assign(user, $store.state.user)">取消</v-btn>
...
export default {
  ...
  watch: { 'dialog.show' () { this.$refs.dialog.reset() }},
  mounted () {
    if (this.user.username === this.$store.state.user.username) {
      this.user = this.$store.state.user
      this.$store.commit('title', this.user.username)
    } else this.$ajax.get(this.$store.state.api.user, { params: { name: this.user.username }})
      .then(resp => {
        if (resp.data.success) {
          // 防止快速返回其他页面时标题被改变
          if (this.$route.name === 'user') this.$store.commit('title', this.user.username)
          this.user = resp.data
        } else this.$store.commit('message', { type: 'error', message: resp.data.message })
      })
      .catch(() => this.$store.commit('message', { type: 'error', message: '获取信息失败' }))
  },
  ...
  methods: {
    update () {
      ...
      if (this.password.new) this.$store.dispatch('auth', {
        ...
        callback: () => Object.assign(this.user, { password: this.password.new })
      })

      this.$ajax.put(this.$store.state.api.user, this.user, { headers: { Authorization: localStorage.getItem('token') }})
        .then(resp => {
          if (resp.data.success) this.$store.commit('message', { type: 'done', message: '修改成功' })
          else this.$store.commit('message', { type: 'error', message: resp.data.message })
        })
        .catch(() => this.$store.commit('message', { type: 'error', message: '修改失败' }))
    },
    deactivate () {
      this.$store.dispatch('auth', {
        username: this.user.username,
        password: this.password.old,
        callback: () => this.$ajax.delete(this.$store.state.api.user, {
          ...
          if (resp.data.success) {
            ...
            this.$store.commit('message', { type: 'done', message: '删除账号成功，期待您的归来' })
          } else this.$store.commit('message', { type: 'error', message: resp.data.message })
        })
          .catch(() => this.$store.commit('message', { type: 'error', message: '删除账号失败' }))
      })
    }
  }
}
