...
export default {
  ...
  methods: {
    publish () {
      this.$ajax.post(this.$store.state.api.activity, this.activity, { headers: { Authorization: localStorage.getItem('token') }})
        .then(resp => {
          if (resp.data.success) {
            this.$emit('close')
            this.$router.push({ name: 'activity', params: { id: resp.data.id }})
            this.$store.commit('message', { type: 'done', message: '发布成功' })
          } else this.$store.commit('message', { type: 'error', message: resp.data.message })
        })
        .catch(() => this.$store.commit('message', { type: 'error', message: '发布失败' }))
    }
  }
}
</script>
